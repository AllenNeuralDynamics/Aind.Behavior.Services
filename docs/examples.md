...
Behavior parameters live in a `TaskParameters` subclass; the concrete task subclasses `Task`, pins `version` / `name`, and points `task_parameters` at it. Stochastic values are expressed with the `distributions` library. Adapted from `Aind.Behavior.Telekinesis` (`src/aind_behavior_telekinesis/task_logic.py`):
...
These `*.json` files are exactly what the launcher hands to Bonsai and what is snapshotted into a session's `Logs/` folder (see [articles](articles/index.md)).


## Calibrating a device

Calibrations are models too. `calibrate_water_valves()` fits a linear regression over measurements and returns a populated `WaterValveCalibration` (`src/aind_behavior_services/rig/water_valve.py`; usage in `examples/water_valve.py`):
...
The resulting calibration slots into the rig model's `calibration` field (see [Defining a Rig](#defining-a-rig)).


## Regenerating schemas and C#

Each repo ships a `regenerate.py` that unions its models into a `pydantic.RootModel` and calls `convert_pydantic_to_bonsai()`, which writes both the JSON Schema and the C# Bonsai serializers. This is the step that keeps Python and the Bonsai runtime in lockstep (see [architecture](architecture.md)). Adapted from `Aind.Behavior.Telekinesis` (`src/aind_behavior_telekinesis/regenerate.py`):
...
!!! note
    `SoftwareEvent` files use the `.jsonl` extension ([#230](https://github.com/AllenNeuralDynamics/Aind.Behavior.Services/issues/230)); `contraqctor` abstracts the extension for consumers, so older `.json` datasets keep loading.
